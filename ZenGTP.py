...
#            Z.analyzeStatus = 0
        ...
        if Cmd[0] == 'play':
            if not len(Cmd)==3:
                ...
                Reply('')
                continue
            if not Cmd[1]=='b' and not Cmd[1]=='w' and not Cmd[1]=='W' and not Cmd[1]=='B':
                Print('wrong color')
                Reply('')
                continue
...
#                Print(rastr)
        ...
        if Cmd == ['territory']:
            ...
            for i in range(0, Z.BoardSize):
              ...
              print(prn)
			  #Japanese rule
            #black_prisoner=Z.ZenGetNumBlackPrisoners()
            #white_prisoner=Z.ZenGetNumWhitePrisoners()
            #black_alive, black_capture, black_territory, white_alive, white_capture, white_territory=Z.stat_territory(500,t)
            #black_score_territory = black_territory + 2*black_capture + black_prisoner
            #white_score_territory = white_territory + 2*white_capture + white_prisoner
            #result_score_territory = black_score_territory - white_score_territory
            ...
            #white_score_area = white_alive + white_capture + white_territory
            #result_score_area = black_score_area - white_score_area
            #Print('@%8d %8d %6d %6d %7d %7d %7d %7d%8d %8d %8d %8d' % (\
            #    black_territory,white_territory, \
            #    black_capture, white_capture, \
            #    black_alive, white_alive, \
			#	black_prisoner, white_prisoner, \
			#	black_score_territory,white_score_territory,\
			#	black_score_area,white_score_area))
